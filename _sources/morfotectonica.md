...
La morfotectónica (o tectónica geomorfológica) se enfoca en cómo los procesos tectónicos activos y a gran escala (levantamiento, subsidencia, deformación) construyen la topografía y cómo podemos usar las formas del paisaje para entender y cuantificar esos procesos {cite:p}`burbank_tectonic_2011`.
...
```{figure} images/morfo_craton_escudo_plataforma.svg
:name: fig-craton-escudo-plataforma
:width: 600px

Diferencia entre un escudo (basamento cristalino expuesto) y una plataforma (basamento cubierto por sedimentos subhorizontales). Elaboración propia.
```
...
```{figure} images/Sentinel_Mesa_MonumentValley.jpg
:name: fig-mesa
:width: 600px

Sentinel Mesa, en Monument Valley (Utah/Arizona, EE. UU.), remanente de una meseta aislado por erosión diferencial sobre estratos horizontales. Fotografía: Warren LeMay, [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:Sentinel_Mesa_in_Monument_Valley,_AZ.jpg) (CC BY-SA 2.0).
```
...
```{figure} images/morfo_margen_pasivo.svg
:name: fig-margen-pasivo
:width: 650px

Perfil característico de un margen continental pasivo, con el interior elevado, el Gran Escarpe y la llanura costera. Elaboración propia.
```
...
```{figure} images/morfo_tipos_esfuerzo.svg
:name: fig-tipos-esfuerzo
:width: 700px

Los tres tipos principales de esfuerzo tectónico y las fallas que originan. Elaboración propia con base en {cite:t}`burbank_tectonic_2011`.
```
...
```{figure} images/morfo_rumbo_buzamiento.svg
:name: fig-rumbo-buzamiento
:width: 500px

Rumbo y buzamiento de un plano geológico inclinado. Elaboración propia.
```
...
```{figure} images/morfo_cuesta_hogback_razorback.svg
:name: fig-cuesta-hogback
:width: 700px

Perfiles de cuesta, hogback y razorback según el ángulo de buzamiento de los estratos. Elaboración propia.
```
...
```{figure} images/morfo_laderas_estructurales.svg
:name: fig-laderas-estructurales
:width: 650px

Laderas cataclinal, anaclinal y ortoclinal en relación con el buzamiento de los estratos. Elaboración propia.
```
...
```{figure} images/morfo_patrones_drenaje.svg
:name: fig-patrones-drenaje
:width: 650px

Patrón de drenaje enrejado (trellis) típico de un relieve de cuestas u hogbacks, con drenajes consecuentes y subsecuentes. Elaboración propia.
```
...
```{figure} images/morfo_tipos_bordes_placa.svg
:name: fig-tipos-bordes-placa
:width: 750px

Los tres tipos de bordes de placa activos y las geoformas asociadas: divergente, convergente y transformante. Elaboración propia con base en {cite:t}`burbank_tectonic_2011`.
```

```{figure} images/ISS_EastAfricanRift_Kenya.jpg
:name: fig-valle-rift
:width: 600px

Valle del Rift de África Oriental en Kenia, fotografiado desde la Estación Espacial Internacional. Fotografía astronauta ISS-30, [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:ISS-30_East_African_Rift_Valley_in_Kenya.jpg) (NASA, dominio público).
```

```{figure} images/morfo_horst_graben.svg
:name: fig-horst-graben
:width: 600px

Bloques elevados (horst) y hundidos (graben) limitados por fallas normales, típicos de zonas de rift. Elaboración propia.
```
...
```{figure} images/morfo_convergencia_tipos.svg
:name: fig-convergencia-tipos
:width: 750px

Los tres tipos de convergencia de placas y sus geoformas características. Elaboración propia con base en {cite:t}`burbank_tectonic_2011`.
```
...
```{figure} images/morfo_geoformas_rumbo.svg
:name: fig-geoformas-rumbo
:width: 650px

Geoformas típicas asociadas a fallas de rumbo: drenajes desplazados, lomo de obturación, hondonada de hundimiento y lomo de presión. Elaboración propia.
```
...
```{figure} images/WaterpocketFold_CapitolReef.jpg
:name: fig-paisaje-plegado
:width: 650px

El Waterpocket Fold, en el Parque Nacional Capitol Reef (Utah, EE. UU.), un monoclinal cuya expresión topográfica ilustra el fuerte control que ejerce la estructura plegada sobre el relieve y el drenaje. Fotografía: Bobak Ha'Eri, [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:2008-0914-CapitolReef-WaterpocketFold1.jpg) (CC BY 3.0).
```
...
Dos principales tipos de paisaje plegado existen: jurásico y apalachense. La principal diferencia es el grado de erosión: el paisaje jurásico muestra una correspondencia directa entre estructura y relieve, mientras que el apalachense muestra una inversión del relieve debido a una erosión prolongada y profunda {cite:p}`davis_rivers_1889`.
...
```{figure} images/morfo_paisaje_jurasico.svg
:name: fig-paisaje-jurasico
:width: 600px

Geoformas del paisaje jurásico (relieve conforme): mont anticlinal, combe y cluse. Elaboración propia.
```

#### El Paisaje Apalachense (Relieve Invertido)
Este estilo, nombrado por los Montes Apalaches en Norteamérica, representa una etapa madura o evolucionada de la erosión, donde la relación inicial entre estructura y topografía se ha invertido {cite:p}`davis_rivers_1889`.
...
```{figure} images/morfo_paisaje_apalachense.svg
:name: fig-paisaje-apalachense
:width: 600px

Geoformas del paisaje apalachense (relieve invertido): el antiguo anticlinal es ahora un valle y el antiguo sinclinal es ahora una cresta. Elaboración propia con base en {cite:t}`davis_rivers_1889`.
```

### Isostasia

La isostasia es el principio de flotación aplicado a la corteza terrestre. Sostiene que la corteza "flota" sobre el manto, que se comporta como un fluido muy denso y viscoso a lo largo del tiempo geológico. La composición química de cada corteza determina su densidad y, por lo tanto, su flotabilidad inherente {cite:p}`airy_computation_1855`.
...
```{figure} images/morfo_isostasia.svg
:name: fig-isostasia
:width: 650px

Principio de isostasia: la corteza continental, más gruesa y menos densa, flota con una raíz profunda; la corteza oceánica, delgada y más densa, flota a menor elevación. Elaboración propia con base en {cite:t}`airy_computation_1855`.
```
...
2. **Patrones de Drenaje:** Si observa un patrón de drenaje sobreimpuesto que atraviesa una cordillera plegada sin desviarse, ¿qué nos dice esto sobre la cronología relativa entre el río y el levantamiento de la cordillera?
3. **Ambientes Tectónicos:** Compare las geoformas resultantes de un borde divergente (rift) con las de un borde de rumbo (transformante). ¿Qué importancia tienen los "shutter ridges" para identificar fallas activas?
4. **Isostasia:** Explique por qué el rebote isostático tras el derretimiento de un gran glaciar puede causar que áreas costeras experimenten un descenso aparente del nivel del mar.
5. **Análisis de Campo:** Utilizando el concepto de buzamiento y rumbo, describa cómo identificaría en el campo una estructura de *hogback* frente a una *cuesta*.
